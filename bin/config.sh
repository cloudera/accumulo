...
if [ -z "${GC}" ] ; then
  ...
  if [ -z "${GC}" ] ; then
    echo "Could not infer a GC role. You need to either define the GC env variable, define \"${ACCUMULO_CONF_DIR}/gc\", or make sure \"${ACCUMULO_CONF_DIR}/masters\" is non-empty."
    exit 1
  fi
fi
if [ -z "${MONITOR}" ] ; then
